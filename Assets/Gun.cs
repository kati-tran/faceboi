...
public class Gun : MonoBehaviour
{
    ...
    public void Shoot(string emotion)
    {
        ...
        Debug.Log(PlayerMovement.uniGrounded);
        if(PlayerMovement.uniGrounded == false){
            Physics.Raycast(g.transform.position,g.transform.forward, out hit, range);
        }
        else{
            Physics.Raycast(g.transform.position + g.transform.up*5.7f,g.transform.forward, out hit, range);
        }
